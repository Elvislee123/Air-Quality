<?php
	...
	if ($resultCheck > 0) {
		while ($row['PM10 sensor'] = mysqli_fetch_assoc($result)) {
			foreach ($row as $sensor_10_data) {
				array_push($PM10_sensor, $sensor_10_data['PM10 sensor']);
				array_push($PM10_London, $sensor_10_data['PM10 London']);
				array_push($x_axis_labels, $count);
				++$count;
			}
		}
		while ($row['PM10 London'] = mysqli_fetch_assoc($result)) {
			foreach($row as $London_10_data) {
				array_push($PM10_London, $London_10_data['PM10 London']);
			}
		}
	}
	...
	$x_axis_average_per_day_London = array();
	...
	if ($resultCheck_all > 0) {
		while ($row2['PM10 sensor'] = mysqli_fetch_assoc($results_all)) {
			foreach($row2 as $sensor_10_data2) {
				array_push($array_all, $sensor_10_data2['PM10 sensor']);
				array_push($array_all_London, $sensor_10_data2['PM10 London']);
			}
		}
		$Total_days = intdiv($resultCheck_all,24);
		foreach(range(0,$Total_days-1) as $Day) {
			array_push($x_axis_average_per_day_London, $Day);
			if ($Day === 0) {
...
<body>
	...
	<script>
		...
		const lineChart = new Chart(CHART, {
			...
			data: {
				...
				datasets: [{
					label: 'PM10 quantity at Home',
					fill: true,
					data: <?php echo json_encode($PM10_sensor) ?>,
					backgroundColor: 'rgba(111, 34, 50, 1)',
					hoverBorderColor: "rgba(111, 34, 50, 1)"
				},
				{
					label: 'PM10 quantity in London',
					data: <?php echo json_encode($PM10_London) ?>,
	...
	<script>
		...
		const lineChart2 = new Chart(CHART2, {
			type: 'line',
			data: {
				labels: <?php echo json_encode($x_axis_average_per_day_London) ?>,
				datasets: [{
					label: 'PM10 Average at home per day',
					data: <?php echo json_encode($array_average_per_day); ?>,
					backgroundColor: 'rgba(111, 34, 50, 1)'
				},
				{
					label: 'PM10 Average in London per day',
			...
			options: {
				...
				scales: {
					yAxes:[{
						...
						scaleLabel: {
							display: true,
							labelString: 'PM10 Average per Day',
							fontColor: 'White'
						},
						ticks: {
                                                        fontColor: "White",
